...
#define LOHALF(x) ((SMsmall)((x) & 0xffff))
#define HIHALF(x) ((SMsmall)((x) >> 16 & 0xffff))
#define TOHIGH(x) ((SMsmall)((x) << 16))


int compare(SMbig a, SMbig b)
{
   int i;
   for(i=SMLIB_DIG_LEN-1; i>=0; i--)
   {
	  if(a[i] > b[i])
		  return 1;
	  else if(a[i] < b[i])
...
void add(SMbig w, SMbig u, SMbig v)
{
	int i, flag;
	SMsmall sum, carry, borrow;

	carry = 0;
	for(i=0; i<SMLIB_DIG_LEN; i++)
	{											
		sum = u[i] + v[i] + carry;
		if(sum < u[i])
		{
...
void sub(SMbig w, SMbig u, SMbig v)
{
	int i;
	SMsmall sum, carry, borrow;
...
void mod(SMbig u, SMbig v)
{	
	SMsmall w[SMLIB_DIG_LEN],carry, borrow, sum;
	int i, flag;
...
void mul(SMbig w, SMbig u, SMbig v)
{
	SMsmall clone[2*SMLIB_DIG_LEN];
	SMsmall r0=0, r1=0, r2=0;
	SMsmall x0, x1, y0, y1;
	SMsmall s1, s2, s3, s4;
...
void inv(SMbig x, SMbig y)
{
	SMsmall u[SMLIB_DIG_LEN], v[SMLIB_DIG_LEN], a[SMLIB_DIG_LEN]={1}, c[SMLIB_DIG_LEN]={0};
	SMsmall sum, carry;
	int i, flagu, flag;
	...
	for(flagu=1,i=0; i<SMLIB_DIG_LEN; i++)
	{
		if (u[i] != 0) 
		{
			flagu = 0; 
	...
	while(!flagu)
	{
		while(u[0] % 2 == 0)
		{
			for(i=0; i<SMLIB_DIG_LEN-1; i++)
			{
				u[i] = (u[i] >> 1) | (u[i+1] << 31);
			}
			u[SMLIB_DIG_LEN-1] = u[SMLIB_DIG_LEN-1] >> 1;
			...
			if(a[0] % 2 == 0)
			{
				...
				a[SMLIB_DIG_LEN-1] = a[SMLIB_DIG_LEN-1] >> 1;
			}
			else
			{
				carry = 0;
				...
				a[SMLIB_DIG_LEN-1] = (a[SMLIB_DIG_LEN-1] >> 1) | (carry << 31);
			}
		}
		...
		while(v[0] % 2 == 0)
		{
			...
			v[SMLIB_DIG_LEN-1] = v[SMLIB_DIG_LEN-1] >> 1;
			...
			if(c[0] % 2 == 0)
			{
				...
				c[SMLIB_DIG_LEN-1] = c[SMLIB_DIG_LEN-1] >> 1;
			}
			else
			{
				carry = 0;
				...
				c[SMLIB_DIG_LEN-1] = (c[SMLIB_DIG_LEN-1] >> 1) | (carry << 31);
			}
		}
...
do{												\
	...
	r1 = 0;
	r2 = 0;
	for(k=0; k<=SMLIB_DIG_LEN; k++)
	{	
		for(i=k; i<=SMLIB_DIG_LEN;i++)
		{
			if(mu[i] != 0)
			{
				mul_and_add(r2, r1, r0, mu[i], x[2*SMLIB_DIG_LEN-1+k-i]);
			}
		}
		clone[k] = r0; 
		r0 = r1; 
	...
	clone[SMLIB_DIG_LEN+1] = r0;
	q1 = clone + 1;

	r0 = 0;
	r1 = 0;
	r2 = 0;
	for(k=0; k<SMLIB_DIG_LEN+1; k++)
	{
		for(i=0; i<=k; i++)
		{
...
void modadd(SMbig w, SMbig u, SMbig v, SMbig p)
{
	int i, flag;
	SMsmall sum, borrow, carry;
...
void modsub(SMbig w, SMbig u, SMbig v, SMbig p)
{
	int i;
	SMsmall sum, borrow, carry;
...
void modinv(SMbig x, SMbig y, SMbig p)
{
	SMsmall u[SMLIB_DIG_LEN], v[SMLIB_DIG_LEN], a[SMLIB_DIG_LEN]={1}, c[SMLIB_DIG_LEN]={0};
	SMsmall sum, carry;
	int i, flagu, flag;
...
void mulmodorder(SMbig w, SMbig u, SMbig v)
{
	SMsmall r0=0, r1=0, r2=0;
	SMsmall x0, x1, y0, y1;
	SMsmall s1, s2, s3, s4;
...
do{												\
	...
	for (k=0; k<=2*(SMLIB_DIG_LEN-1); k++)
	{
		i = (k-(SMLIB_DIG_LEN-1) <= 0) ? 0: (k-(SMLIB_DIG_LEN-1));
		j = (k <= SMLIB_DIG_LEN-1) ? k : (SMLIB_DIG_LEN-1);
		while (i <= j)
		{
...
{
	int i, flag;
	SMsmall t1[SMLIB_DIG_LEN], t2[SMLIB_DIG_LEN], lambda[SMLIB_DIG_LEN];
	
	for(flag=1,i=0; i<SMLIB_DIG_LEN; i++)
	{
		if ((p->x[i] != 0) || (p->y[i] != 0)) 
		{
			flag = 0; 
...
void pointmul(epoint p, epoint q, SMbig n)
{
	int naf[M + 1]={0};
	int lnaf, flag, i, j, l;
	int flag1, flag2;
	SMsmall carry, k[SMLIB_DIG_LEN];
	projpoint pre[8], temp;
	projpoint clone={{1},{1},{0}};

	for (flag1=1,i=0; i<SMLIB_DIG_LEN; i++)
	{
		if (n[i] != 0) 
		{
			flag1 = 0; 
	...
	for (flag2=1,i=0; i<SMLIB_DIG_LEN; i++)
	{
		if ((q->x[i] != 0) || (q->y[i] != 0)) 
		{
			flag2 = 0; 
	...
	{	
		if (k[0] % 2 == 0)
		{
			...
			for (j=0; j<SMLIB_DIG_LEN-1; j++)
			{
				k[j] = (k[j] >> 1) | (k[j+1] << 31);
			}
			k[SMLIB_DIG_LEN-1] = k[SMLIB_DIG_LEN-1] >> 1;
		}
		else
		{
			flag = k[0] & 0x1f;
			...
			k[SMLIB_DIG_LEN-1] = k[SMLIB_DIG_LEN-1] >> 5;

			for (flag1=1,j=0; j<SMLIB_DIG_LEN; j++)
			{
				if (k[j] !=0 ) 
				{
					flag1 = 0; 
	...
	for (j=lnaf-1; j>=0; j--)
	{
		...
		if (naf[j] != 0)
		{
			...
			if (naf[j] < 0)
			{
				...
				sub(temp.y, (SMsmall *)P, pre[i].y);
				for (l=0; l<SMLIB_DIG_LEN; l++)
				{
					temp.x[l] = pre[i].x[l]; 
					temp.z[l] = pre[i].z[l];
				}
...
void projpointdouble(point p, point q)
{
	SMsmall t1[SMLIB_DIG_LEN], t2[SMLIB_DIG_LEN], t3[SMLIB_DIG_LEN], t4[SMLIB_DIG_LEN], t5[SMLIB_DIG_LEN];
	int i, flag1, flag2;
...
void mixpointadd(point r, point p, epoint q)
{
	SMsmall t1[SMLIB_DIG_LEN], t2[SMLIB_DIG_LEN], t3[SMLIB_DIG_LEN], t4[SMLIB_DIG_LEN];
	SMsmall t5[SMLIB_DIG_LEN], t6[SMLIB_DIG_LEN];
	SMsmall sum, carry;
	int i, flag1, flag2;
	...
	if (t2[0] % 2 == 0) 
	{
		...
		r->y[SMLIB_DIG_LEN-1] = t2[SMLIB_DIG_LEN-1] >> 1;
	...
	else
	{
		...
		r->y[SMLIB_DIG_LEN-1] = (t2[SMLIB_DIG_LEN-1] >> 1) | (carry << 31);
	}
}
...
{
	SMsmall t1[SMLIB_DIG_LEN], t2[SMLIB_DIG_LEN], t3[SMLIB_DIG_LEN], t4[SMLIB_DIG_LEN];
	SMsmall t5[SMLIB_DIG_LEN], t6[SMLIB_DIG_LEN], t7[SMLIB_DIG_LEN];
	SMsmall sum, carry;
	int i, flag1, flag2;
...
void basepointmul(epoint p, SMbig n)
{
...
{
	int i;
	SMsmall g[SMLIB_DIG_LEN], h[SMLIB_DIG_LEN],l[SMLIB_DIG_LEN], m;
...
{
	int i, j;
	unsigned int prikey[SMLIB_DIG_LEN]={0};
	unsigned int x;
	affpoint pubkey;

	if (sk_len != 4*SMLIB_DIG_LEN) 
	{
		return(-1);
	}

	for (i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		prikey[SMLIB_DIG_LEN-1-i] = (((unsigned int)(sk[j]) << 24) | ((unsigned int)(sk[j+1]) << 16)
	...
	for (i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		pk[j] = (unsigned char)((pubkey.x[SMLIB_DIG_LEN-1-i] >> 24) & 0xff);
		pk[j+1] = (unsigned char)((pubkey.x[SMLIB_DIG_LEN-1-i] >> 16) & 0xff);
		pk[j+2] = (unsigned char)((pubkey.x[SMLIB_DIG_LEN-1-i] >> 8) & 0xff);
		pk[j+3] = (unsigned char)((pubkey.x[SMLIB_DIG_LEN-1-i]) & 0xff);

		pk[j+4*SMLIB_DIG_LEN] = (unsigned char)((pubkey.y[SMLIB_DIG_LEN-1-i] >> 24) & 0xff);
		pk[j+1+4*SMLIB_DIG_LEN] = (unsigned char)((pubkey.y[SMLIB_DIG_LEN-1-i] >> 16) & 0xff);
		pk[j+2+4*SMLIB_DIG_LEN] = (unsigned char)((pubkey.y[SMLIB_DIG_LEN-1-i] >> 8) & 0xff);
		pk[j+3+4*SMLIB_DIG_LEN] = (unsigned char)((pubkey.y[SMLIB_DIG_LEN-1-i]) & 0xff);
	}

	*pk_len = 8*SMLIB_DIG_LEN;
			...
			unsigned char *sign, unsigned int *sign_len)
{
	int i, j;
	unsigned int prikey[SMLIB_DIG_LEN] = {0};
	unsigned int rand[SMLIB_DIG_LEN] = {0};
	unsigned int e[SMLIB_DIG_LEN] = {0};
	unsigned int r[SMLIB_DIG_LEN] = {0};
	unsigned int s[SMLIB_DIG_LEN] = {0};
	unsigned int y[SMLIB_DIG_LEN] = {0};
	unsigned int h[SMLIB_DIG_LEN] = {0};
	unsigned int l[SMLIB_DIG_LEN] = {0};
	unsigned int m[SMLIB_DIG_LEN] = {0x1};
	unsigned int z[SMLIB_DIG_LEN] = {0x1};
	affpoint kg;
	unsigned int x;

	if (hash_len != 4*SMLIB_DIG_LEN)
	{
		return(-1);
	}
	if (random_len != 4*SMLIB_DIG_LEN)
	{
		return(-1);
	}
	if (sk_len != 4*SMLIB_DIG_LEN)
	...
	for (i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		rand[SMLIB_DIG_LEN-1-i] = (((unsigned int)(random[j]) << 24) | ((unsigned int)(random[j+1]) << 16)
	...
	for (i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		e[SMLIB_DIG_LEN-1-i] = (((unsigned int)(hash[j]) << 24) | ((unsigned int)(hash[j+1]) << 16)
    ...
    modadd(y, m, prikey, (SMsmall*)N);
	modinv(h, y, (SMsmall*)N);
	mulmodorder(y, r, prikey);
	modsub(l, rand, y, (SMsmall*)N);
	mulmodorder(s, h, l);
	...
	for (i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		sign[j] = (unsigned char)((r[SMLIB_DIG_LEN-1-i] >> 24) & 0xff);
		sign[j+1] = (unsigned char)((r[SMLIB_DIG_LEN-1-i] >> 16) & 0xff);
		sign[j+2] = (unsigned char)((r[SMLIB_DIG_LEN-1-i] >> 8)&0xff);
		sign[j+3] = (unsigned char)((r[SMLIB_DIG_LEN-1-i]) & 0xff);

		sign[j+4*SMLIB_DIG_LEN] = (unsigned char)((s[SMLIB_DIG_LEN-1-i] >> 24) & 0xff);
		sign[j+1+4*SMLIB_DIG_LEN] = (unsigned char)((s[SMLIB_DIG_LEN-1-i] >> 16) & 0xff);
		sign[j+2+4*SMLIB_DIG_LEN] = (unsigned char)((s[SMLIB_DIG_LEN-1-i] >> 8) & 0xff);
		sign[j+3+4*SMLIB_DIG_LEN] = (unsigned char)((s[SMLIB_DIG_LEN-1-i]) & 0xff);
	}

	*sign_len = 8 * SMLIB_DIG_LEN;
...
{
	int i, j;
	unsigned int t[SMLIB_DIG_LEN] = {0};
	unsigned int r[SMLIB_DIG_LEN] = {0};
	unsigned int s[SMLIB_DIG_LEN] = {0};
	unsigned int e[SMLIB_DIG_LEN] = {0};
	unsigned int R[SMLIB_DIG_LEN] = {0};
	...
	if (pk_len != 8*SMLIB_DIG_LEN) 
	{
		return(-1);
	}
	if (sign_len != 8*SMLIB_DIG_LEN) 
	{
		return(-1);
	}
	...
	for (i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		p1.x[SMLIB_DIG_LEN-1-i] = (((unsigned int)(pk[j]) << 24) | ((unsigned int)(pk[j+1]) << 16)
					| ((unsigned int)(pk[j+2])<<8) | ((unsigned int)(pk[j+3])));
		p1.y[SMLIB_DIG_LEN-1-i] = (((unsigned int)(pk[j+4*SMLIB_DIG_LEN]) << 24) | ((unsigned int)(pk[j+1+4*SMLIB_DIG_LEN]) << 16)
					| ((unsigned int)(pk[j+2+4*SMLIB_DIG_LEN]) << 8) | ((unsigned int)(pk[j+3+4*SMLIB_DIG_LEN])));
	}

	for (i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		r[SMLIB_DIG_LEN-1-i] = (((unsigned int)(sign[j]) << 24) | ((unsigned int)(sign[j+1]) << 16)
					| ((unsigned int)(sign[j+2]) << 8) | ((unsigned int)(sign[j+3])));
		s[SMLIB_DIG_LEN-1-i] = (((unsigned int)(sign[j+4*SMLIB_DIG_LEN]) << 24)|((unsigned int)(sign[j+1+4*SMLIB_DIG_LEN]) << 16)
					| ((unsigned int)(sign[j+2+4*SMLIB_DIG_LEN]) << 8)|((unsigned int)(sign[j+3+4*SMLIB_DIG_LEN])));
	}
	...
	x = compare(s, (SMsmall*)N);
	if(x == 1)
	{
		return(-1);
	}
    
	modadd(t, r, s, (SMsmall*)N);
				...
				unsigned char *cipher, unsigned int *cipher_len)
{
	...
	unsigned int y, rand[SMLIB_DIG_LEN] = {0};
	affpoint p;
	unsigned char x[8*SMLIB_DIG_LEN], t[96];
	...
	for (i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		cipher[j] = (unsigned char)((p.x[SMLIB_DIG_LEN-1-i] >> 24) & 0xff);
		cipher[j+1] = (unsigned char)((p.x[SMLIB_DIG_LEN-1-i] >> 16) & 0xff);
		cipher[j+2] = (unsigned char)((p.x[SMLIB_DIG_LEN-1-i] >> 8) & 0xff);
		cipher[j+3] = (unsigned char)((p.x[SMLIB_DIG_LEN-1-i]) & 0xff);
			  
		cipher[j+4*SMLIB_DIG_LEN] = (unsigned char)((p.y[SMLIB_DIG_LEN-1-i] >> 24) & 0xff);
		cipher[j+1+4*SMLIB_DIG_LEN] = (unsigned char)((p.y[SMLIB_DIG_LEN-1-i] >> 16) & 0xff);
		cipher[j+2+4*SMLIB_DIG_LEN] = (unsigned char)((p.y[SMLIB_DIG_LEN-1-i] >> 8) & 0xff);
		cipher[j+3+4*SMLIB_DIG_LEN] = (unsigned char)((p.y[SMLIB_DIG_LEN-1-i]) & 0xff);
	}
			  
	for (i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		p.x[SMLIB_DIG_LEN-1-i] = (((unsigned int)(pk[j]) << 24) | ((unsigned int)(pk[j+1]) << 16)
								  | ((unsigned int)(pk[j+2]) << 8) | ((unsigned int)(pk[j+3])));
		p.y[SMLIB_DIG_LEN-1-i] = (((unsigned int)(pk[j+4*SMLIB_DIG_LEN]) << 24) | ((unsigned int)(pk[j+1+4*SMLIB_DIG_LEN]) << 16)
								  | ((unsigned int)(pk[j+2+4*SMLIB_DIG_LEN]) << 8) | ((unsigned int)(pk[j+3+4*SMLIB_DIG_LEN])));
	}
	pointmul(&p, &p, rand);
	for (i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		x[j] = (unsigned char)((p.x[SMLIB_DIG_LEN-1-i] >> 24) & 0xff);
		x[j+1] = (unsigned char)((p.x[SMLIB_DIG_LEN-1-i] >> 16) & 0xff);
		x[j+2] = (unsigned char)((p.x[SMLIB_DIG_LEN-1-i] >> 8) & 0xff);
		x[j+3] = (unsigned char)((p.x[SMLIB_DIG_LEN-1-i]) & 0xff);
			  
		x[j+4*SMLIB_DIG_LEN] = (unsigned char)((p.y[SMLIB_DIG_LEN-1-i] >> 24) & 0xff);
		x[j+1+4*SMLIB_DIG_LEN] = (unsigned char)((p.y[SMLIB_DIG_LEN-1-i] >> 16) & 0xff);
		x[j+2+4*SMLIB_DIG_LEN] = (unsigned char)((p.y[SMLIB_DIG_LEN-1-i] >> 8) & 0xff);
		x[j+3+4*SMLIB_DIG_LEN] = (unsigned char)((p.y[SMLIB_DIG_LEN-1-i]) & 0xff);
	}
				  
	// 加密
	KDF(x, pk_len, plain_len,cipher+8*SMLIB_DIG_LEN);
				  
	y = 0; 
	for (i=0; i<(int)plain_len; i++) 
	{
		y |= cipher[8*SMLIB_DIG_LEN+i];
	}
	if (y == 0) 
	{
		return(-1);
	...
	for (i=0; i<(int)plain_len; i++)
	{   
		cipher[8*SMLIB_DIG_LEN+i] ^= plain[i];
	}
	if(plain_len < 33)
	{
		for(i=0; i<32; i++)
	...
	else
	{
		...
		for(i=0; i<32; i++)
			t[32+i] = plain[i];
		SM3_HASH(t, 64, 64+plain_len, cipher+8*SMLIB_DIG_LEN+plain_len, 32, 1);
		multiple = (plain_len-32)/64;
		remainder = (plain_len-32)%64;
		if(multiple != 0)
			SM3_HASH(plain+32, 64*multiple, 64+plain_len, cipher+8*SMLIB_DIG_LEN+plain_len, 32, 2);
	...
	cipher_len[0] = 8*SMLIB_DIG_LEN+plain_len+32;
			...
			unsigned char *plain, unsigned int *plain_len)
{
    ...
    // 判断密文是否有效，无效则返回错误
	if((int)cipher_len < 8*SMLIB_DIG_LEN+32)
	{
		return(-1);
	}
	// 判断私钥是否有效，无效则返回错误
	...
	for(i=0,j=0; i<SMLIB_DIG_LEN; i++,j+=4)
	{
		p.x[SMLIB_DIG_LEN-1-i] = (((unsigned int)(cipher[j]) << 24) | ((unsigned int)(cipher[j+1]) << 16)
						| ((unsigned int)(cipher[j+2]) << 8) | ((unsigned int)(cipher[j+3])));
		p.y[SMLIB_DIG_LEN-1-i] = (((unsigned int)(cipher[j+4*SMLIB_DIG_LEN]) << 24) | ((unsigned int)(cipher[j+1+4*SMLIB_DIG_LEN]) << 16)
						| ((unsigned int)(cipher[j+2+4*SMLIB_DIG_LEN]) << 8) | ((unsigned int)(cipher[j+3+4*SMLIB_DIG_LEN])));
	}
