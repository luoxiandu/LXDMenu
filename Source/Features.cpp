...
void Features::OnlinePlayerInfo(char* playerName, Player p)
{
	...
	drawText("名字:", 0, 0.48f, 0.043f, 0.440f, 0.40f, bannerTextRed, bannerTextGreen, bannerTextBlue, bannerTextOpacity, false);
	//client Level
	int Client_Rank = Read_Global(1581952 + (306 * Features::Online::selectedPlayer));
	char *ClientRank_string = ItoS(Client_Rank);
	char Rank_buf[90];
	snprintf(Rank_buf, sizeof(Rank_buf), "等级: ~b~%s", ClientRank_string);
	...
	char KD_buf[90];
	snprintf(KD_buf, sizeof(KD_buf), "杀敌数: ~b~%s", KD_String);
	...
	char *alivetext = "";
	if (isAlive == true) { alivetext = "存在: ~b~Yes"; }
	else { alivetext = "存在: ~r~No"; }
	...
	char Health_buf[90];
	snprintf(Health_buf, sizeof(Health_buf), "生命值: ~b~%s", Health);
	...
	PATHFIND::GET_STREET_NAME_AT_COORD(coords.x, coords.y, coords.z, &StreetName, &CrossingRoad);
	snprintf(STreetN_buffer, sizeof(STreetN_buffer), "街道位置: ~b~%s ", UI::GET_STREET_NAME_FROM_HASH_KEY(StreetName));
	...
	snprintf(RoadCross_Buff, sizeof(RoadCross_Buff), "道路交叉点: ~b~%s ", UI::GET_STREET_NAME_FROM_HASH_KEY(CrossingRoad));
