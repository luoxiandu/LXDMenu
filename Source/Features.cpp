...
bool wallDrive = 0, Rainb = 0, Drift = 0, Gravity = 0, AirStrike = 0, forceog = 0, Rneon = 0, ShootR = 0, Trigger = 0, Superr = 0,
...
void Features::UpdateLoop()
{
	...
	LoopKill ? loopKill(true) : NULL;

	ShakeLobby ? shakeLobby(true) : NULL;

	Crash ? crash(true) : NULL;

	FreezeA ? freeze(true) : NULL;

	AntiFx ? antiFx(true) : NULL;

	AntiCrash ? antiCrash(true) : NULL;

	Stealtha ? stealtha(true) : NULL;

	Stealthb ? stealthb(true) : NULL;

	Stealthc ? stealthc(true) : NULL;

	Stealthd ? stealthd(true) : NULL;

	Stealtha ? stealthe(true) : NULL;

	camshaker ? shakecam(true) : NULL;

	exploder ? explodeloop(true) : NULL;
...
bool Features::LoopKill = false; 
void Features::loopKill(bool toggle)
{
	if (toggle == TRUE)
	{
		...
		for (int i = 0; i <= 32; i++)
		{
			...
			FIRE::ADD_EXPLOSION(pos.x, pos.y, pos.z, 29, 5.5f, true, false, 50.0f);
...
bool Features::ShakeLobby = false;
void Features::shakeLobby(bool toggle)
{
	if (toggle == TRUE)
	{
		Ped playerPedID = PLAYER::PLAYER_PED_ID();
		...
		for (int i = 0; i <= 32; i++)
		{
			...
			FIRE::ADD_EXPLOSION(pos.x, pos.y, pos.z, +10.0f, 0, 2000, false, false);
			{
				if (i == 32)
				{
					break;
...
bool Features::Crash = false; 
void Features::crash(bool toggle)
{
	if (toggle == TRUE)
	{
		for (int i = 0; i <= 32; i++)
	...
	bool Features::FreezeA = false;
	void Features::freeze(bool toggle)
	{
		for (int i = 0; i < 32; i++)
		{
...
bool Features::AntiFx = false;
void Features::antiFx(bool toggle)
{
}

bool Features::AntiCrash = false;
void Features::antiCrash(bool toggle)
{
	
}
bool Features::Stealtha = false;
void Features::stealtha(bool toggle)
{

}
bool Features::Stealthb = false;
void Features::stealthb(bool toggle)
{
}
bool Features::Stealthc = false;
void Features::stealthc(bool toggle)
{

}
bool Features::Stealthd = false;
void Features::stealthd(bool toggle)
{

}
bool Features::Stealthe = false;
void Features::stealthe(bool toggle)
{
}
...
bool Features::TinyPlayer = false;
...
bool Features::Online::WaterLoop  = false;
void Features::Online::waterLoop(Player target)
{
...
bool Features::Online::FreezeLoop = false;
void Features::Online::FreezeLooop(Player target)
{
	AI::CLEAR_PED_TASKS_IMMEDIATELY(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(Features::Online::selectedPlayer));
}
bool Features::Online::Explode = false;
void Features::Online::Explodee(Player target)
{
	Vector3 coords = ENTITY::GET_ENTITY_COORDS(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(Features::Online::selectedPlayer), false);
	FIRE::ADD_EXPLOSION(coords.x, coords.y, coords.z, 28, 10.5f, true, false, 1.f);
...
void Features::Rainbowmenu(bool toggle)
{
	...
	if (Menu::Settings::line.b > 0 && Menu::Settings::line.g == 0, Menu::Settings::line.b > 0 && Menu::Settings::line.g == 0)
	{
...
void Features::maxvehicle()
{
	...
	VEHICLE::SET_VEHICLE_NUMBER_PLATE_TEXT(vehicle, "LULA2018");
}
...
void Features::shootAtPed(Player selectedPlayer)
{
